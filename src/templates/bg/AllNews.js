  ...
  render() {
    ...
    const news = this.props.api.map((filters, index) =>
      <div className="news-box-content" key={index}>
        <Link onClick={this.newsClick} className="all-news-img" to={{pathname: filters.subtitle_bg}}>
          <img value={index} src={"https://dev.winbet-bg.com/uploads/images/news/" + filters.image_name} alt="winbet news"/>
        </Link>
        <div className="all-news-content">
          ...
          <Link onClick={this.newsClick} value={index} className="all-news-link"  to={{pathname: filters.subtitle_bg}}>
          {this.props.read}
          </Link>
        </div>
      </div>
